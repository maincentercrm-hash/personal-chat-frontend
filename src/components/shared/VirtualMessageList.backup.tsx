...
const VirtualMessageList = forwardRef<VirtualMessageListRef, VirtualMessageListProps>(({
  ...
  const { grouped: groupedAlbums, standalone: _standaloneMessages } = useMemo(() => {
    const result = groupMessagesByAlbum(deduplicatedMessages);
  ...
  const renderAlbum = useCallback(
    ...
    [isOwnMessage, getFormattedSender, isBusinessView, isGroupChat, formatTime, getMessageStatus, renderMessageStatus, formatMessageStatus, onImageClick]
  );
  ...
  const MessageItem = memo(({ message }: { message: MessageDTO }) => {
    ...
    // ✅ SIMPLIFIED: Album rendering - only first message (position 0) renders the album
    ...
    if (albumId !== undefined && albumPosition !== undefined) {
      // Only the FIRST message (position 0) renders the entire album
      if (albumPosition === 0) {
        const albumMessages = groupedAlbums[albumId];

        if (albumMessages && albumMessages.length > 0) {
          console.log('📸 [Album] Rendering album at position 0:', {
            albumId: albumId.slice(0, 8),
            messageCount: albumMessages.length
          });
        ...
        // Skip other positions (1, 2, 3, ...) - they're included in position 0's album
        console.log('📸 [Album] Skipping position:', {
    ...
    const messageContent = useMemo(() => {
      ...
      return null;
    }, [message, isUser, formattedStatus, formattedSender, onImageClick, scrollToMessageProp, onJumpToMessage]);
    ...
    <div className="flex-1 bg-background min-h-0">
      <Virtuoso
        ...
        itemContent={(_index, message) => {
          ...
          const result = <MessageItem message={message} />;

          // 🔍 DEBUG: Log what itemContent returns
          if (message.metadata?.album_id) {
            console.log('📸 [Virtuoso itemContent] Rendering message:', {
              index: _index,
              messageId: message.id?.slice(0, 8),
              albumId: message.metadata.album_id.slice(0, 8),
              position: message.metadata.album_position,
